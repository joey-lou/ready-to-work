...
Update state.json at {state_path}: read it, set only plan_status (IN_PROGRESS|COMPLETED|BLOCKED) and blocking_reason.
...
Outputs: edit {plan_path}, {subtask_path}.
If complete, write SUMMARY.md.
Update {state_path}: read it, set only plan_status and blocking_reason. Preserve all other keys. Write state.json back.
...
Update state.json at {state_path}: read it, set only subtask_status (REVISE|PASSED|BLOCKED) and blocking_reason.
Preserve all other keys.
Do NOT summarize or repeat in your response; only edit files.
...
# Iteration {iteration} of {max_iter}
Outputs: edit {subtask_path}.
Update {state_path}: read it, set only subtask_status and blocking_reason. Preserve all other keys. Write state.json back.
"""
